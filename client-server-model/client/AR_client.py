...
logging.info(
    f'----------------- Client started at {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())} -----------------'
)
...
def main():
    ...
    aux_list: deque = receiveMessage(CLIENT, _type="dict")
    logging.debug(f"Received auxiliary list successfully!")
    al_access_requests = []
    req_template = {"sub": {"uid": []}, "obj": {"rid": []}, "op": "read"}
    # for obj_attr in obj_attr_val:
    #     req_template["obj"][obj_attr] = ["*"]
    # for sub_attr in user_attr_val:
    #     req_template["sub"][sub_attr] = ["*"]
    for i in range(len(aux_list)):
        sub, obj, op = aux_list[i]
        req = req_template.copy()
        req["sub"]["uid"].append(sub)
        req["obj"]["rid"].append(obj)
        req["op"] = op
        result = resolveAR(req, policy, 0)
        logging.debug(f"Fixed auxlist Access request {i} {'granted' if result == 1 else 'denied'}")
        al_access_requests.append(req)
    ...
    obj_choice = random.choices([0, 1, 2], weights=[0.3, 0.4, 0.3], k=50005)
    req_sent = [0, 0, 0]
    ...
    try:
        logging.debug(
            f"Starting the sending of access requests with arrival rate: {AR_ARRIVAL_RATE}"
        )
        while inputs:
            ...
            if not (read_ready or write_ready or exception):
                ...
                for sub_attr in user_attr_val:
                    access_request["sub"][sub_attr] = random.sample(
                        user_attr_val[sub_attr] + ["*"], 1
                    )[0]
                for obj_attr in obj_attr_val:
                    access_request["obj"][obj_attr] = random.sample(
                        obj_attr_val[obj_attr] + ["*"], 1
                    )[0]
                AR_send = {}
                match obj_choice[ar_count - 1]:
                    case 0:
                        AR_send = access_request
                    case 1:
                        AR_send = policy[
                            "rule_" + str(random.randint(1, tot_len_of_policy))
                        ]
                    case 2:
                        AR_send = random.choice(al_access_requests)
                req_sent[obj_choice[ar_count - 1]] += 1
                file_ptr.write(str(AR_send) + "\n")
                # result = resolveAR(AR_send, policy, ar_count)
                # logging.info(f"-- ACCESS REQUEST {ar_count} on client side with choice[{obj_choice[ar_count - 1]}] --")
                # if result == 1:
        ...
        logging.info(f"Total access requests sent: {ar_count}")
        logging.info("Requests per mode: ")
        logging.info(f"Random: {req_sent[0]}")
        logging.info(f"Policy: {req_sent[1]}")
        logging.info(f"Auxiliary: {req_sent[2]}")
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "-a", "--arrival_rate", type=int, help="Mean arrival rate of access requests"
    )
